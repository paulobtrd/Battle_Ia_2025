...
namespace BattleIAserver
{
    public class OneBot
    {
            ...
            while (!result.CloseStatus.HasValue)
            {
                ...
                if (State == BotState.WaitingGUID)
                {
                    ...
                    if (Guid.TryParse(text, out bot.GUID))
                    {
                        ...
                        MapXY xy = PlaceBotOnMap();
                        bot.Energy = MainGame.Settings.EnergyStart;
                        // IndestructibleBot : Hack :)
                        if (MainGame.Settings.IndestructibleBot && bot.GUID.ToString() == MainGame.Settings.IndestructibleBotId)
                        {
                            bot.Energy = 50000;
                        }
                        bot.Score = 0;
        ...
        public async Task<UInt16> IsHit()
        {
            ...
            if (bot.ShieldLevel > 0)
            {
                UInt16 hitValue = (ushort)(MainGame.Settings.PointByEnnemyTouch / MainGame.Settings.ShieldAbsorptionRatio);
                // le bouclier absorbe le choc
                if (bot.ShieldLevel >= hitValue)
                {
                    bot.ShieldLevel -= hitValue;
                }
                else
                {
                    hitValue -= bot.ShieldLevel;
                    // points = (ushort)(MainGame.Settings.PointByEnnemyTouch - bot.ShieldLevel);
                    points = (ushort)(hitValue * MainGame.Settings.ShieldAbsorptionRatio);
                    bot.ShieldLevel = 0;
